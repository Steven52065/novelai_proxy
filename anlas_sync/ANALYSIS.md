...
## 12. 两次同步的经验教训（2026-08-22 网页大改）

本文档第一次抓取（2026-08-18）后官网迎来一次较大改版：定价模块换了 chunk 和模块 id，
变量名也变了。以下是本次处理中沉淀下来的经验，供下次改版直接复用。

### 12.1 怎么判断“网页大改”而不是脚本小毛病

- `extract.py` 报 `模块 xxx 未找到，网页结构可能已变化` 是最直接的信号。
- `download.py` 报“未找到 chunk 前缀”说明 chunk 文件改名了。
- 两者同时出现基本可以断定是前端重构，按第 10 节重新定位，不要在原正则上硬凑。

### 12.2 TLS 指纹：下载失败先怀疑 CDN 拦截

- 第一次用 httpx 能下载；本次 httpx 报 `SSL: UNEXPECTED_EOF_WHILE_READING`，
  提升网络权限也一样。这是 novelai.net 的 CDN 按 TLS 指纹拦截，不是网络问题。
- 解法：`curl_cffi.requests` + `impersonate="chrome136"`（项目上游本来就依赖 curl_cffi）。
- 经验：`download.py` 若再出现 SSL 类报错，优先怀疑指纹拦截；先用手动小脚本
  `requests.get(url, impersonate="chrome136")` 验证，再改脚本。不要轻易加
  `verify=False` 之类的绕过（curl_cffi 的 impersonate 已足够）。

### 12.3 定位新模块：特征常量比关键词更可靠

- 搜索 `anlas` / `getPrice` 命中很多无关模块（购买弹窗、错误处理等）。
- 用**计费公式的特征常量**最精准：`15.266497014243718`（经典公式系数）或
  `0.6326248927474729`，一次就能命中定价模块所在文件。
- 拿到文件后用 `save_script_source` 保存整份 chunk，再在本地用 Python 正则分析，
  比在浏览器里一屏屏翻 minified 代码高效得多。

### 12.4 本次模块迁移记录（下次可直接对照）

| 旧（2026-08-18） | 新（2026-08-22） | 说明 |
|---|---|---|
| chunk 2075 模块 61094 | chunk 7416 模块 25690 | SW.getPrice，函数体几乎没变 |
| chunk 3811 模块 85341 | chunk 1741 | image 页主组件，角色引用 5/个/张 没变 |
| _app 模块 23379 | chunk 1052 模块 61225 | 定价 GI/tY/H_；Dk 拆到 57863 |
| _app 模块 18401 | _app 模块 53856 | 枚举/Jg/PE/VI |
| _app 模块 71810 | _app 模块 62654 | ax 订阅判断 |
| _app 模块 1018 | _app 模块 46542 | 常量 dZ/kJ/Hi |
| （新增） | _app 模块 41179 | 免费小图 t1 判断 |

经验：即使导出名（GI/tY/H_/Dk/ax/Jg）没变，模块 id 和所在 chunk 也可能变；
更新 `extract.py` 与 `oracle.js` 时两处要一起改，否则对拍会一直失败。

### 12.5 本次踩过的提取正则坑（下次重点检查）

1. **数组在同一个 let 链里**：`let i=[...],o=[...],l=[...],s=[...],d=[...];`
   直接全局搜 `,o=[` 可能误匹配，先截 `let i=[` 到 `;var u=` 的片段再逐个取。
2. **`g=[[...]]` 捕获组末项没有闭合 `]`**：`\[\[(.*?)\]\]` 抓到的是
   `1048576,1],...[c.xM,4`（最后一个 `]` 被外层 `]]` 吃掉），解析末项必须用
   `(?:\]|$)`；`c.xM` 要另从模块 57863 的 `m=3145728` 解析，不能直接 `int()`。
3. **PE 的多标签共享 return**：v5 的 4 个模型共用同一个 `return{...opusUsageLimit:!0}`，
   按单个 case 切块会漏掉 3 个模型。必须用
   `(?:case"([^"]+)":)+return\{...opusUsageLimit:(!0|!1)` 整组匹配再收集全部标签。
4. **SW.getPrice 取最后一个价格**：函数体前面有 `{exists:!1,price:0}` 的兜底，
   未编码分支的 `price:2` 在最后，`findall` 后取 `[-1]`。
5. **枚举/Set 要认准变量名**：VI_SET 现在是 `let C=new Set(...)`，不要取
   “最后一个含采样器的 Set”，模块里可能还有别的 Set。

### 12.6 对拍与验证的坑

- `compare.py` 多个 seed **不能并发跑**：它们共用 `generated/_compare_in.json` /
  `_compare_out.json`，并发会互相覆盖产生大量假不一致（本次踩到，改成串行即可）。
- 对拍前先单独验证 oracle 可用（跑一个小用例），再上 5000 组。
- 对拍全绿后，把新的已知价格样例（第 11 节）同步更新，方便下次 sanity check。
- 改完记得跑全量 `pytest`：应用侧（app/）也有依赖 pricing_data 的测试。

### 12.7 下次改版的快速清单

1. 浏览器打开页面 → `list_scripts` 拿全部 chunk 名 → 更新 `download.py` NEEDED。
2. 搜特征常量 `15.266497014243718` 定位新定价模块 → 更新 `extract.py`/`oracle.js`。
3. 逐个核对第 6.3 节字段是否仍存在、语义是否变化（本次变了 4 处：v5 倍率、
   放大表、Dk、免费小图 opus 条件）。
4. 串行跑 2~3 个 seed 对拍 + `pytest`，全绿后更新 ANALYSIS.md 再提交。
