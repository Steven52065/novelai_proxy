...
def _create_user(client: TestClient, name: str, **overrides) -> int:
    payload = {"name": name, "tier": "normal", "anlas_total": 100, "reset_period": "month", "reset_day": 5}
...
def test_bulk_reset_anlas_clears_usage_without_touching_schedule(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("NOVELAI_PROXY_CONFIG", str(write_test_config(tmp_path)))
    from app.main import app

    with TestClient(app) as client:
        first_id = _create_user(client, "bulk-anlas-1")
        second_id = _create_user(client, "bulk-anlas-2")
        untouched_id = _create_user(client, "bulk-anlas-untouched")
...
def test_bulk_reset_free_small_daily_clears_current_window_only(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("NOVELAI_PROXY_CONFIG", str(write_test_config(tmp_path)))
    from app.main import app
...
def test_bulk_reset_rejects_empty_selection_and_unknown_user(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("NOVELAI_PROXY_CONFIG", str(write_test_config(tmp_path)))
    from app.main import app

    with TestClient(app) as client:
        user_id = _create_user(client, "bulk-validate-user")
        client.app.state.db.execute(
        ...
        for path in ("/admin/api/users/bulk-reset-anlas", "/admin/api/users/bulk-reset-free-small-daily"):
            empty_resp = client.post(path, auth=("admin", "admin123"), json={"user_ids": []})
            # 本项目将请求验证错误统一转换为 400。
            assert empty_resp.status_code == 400

            missing_resp = client.post(path, auth=("admin", "admin123"), json={"user_ids": [user_id, 999]})
...
def test_bulk_reset_web_forms_and_page_controls(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("NOVELAI_PROXY_CONFIG", str(write_test_config(tmp_path)))
    from app.main import app
